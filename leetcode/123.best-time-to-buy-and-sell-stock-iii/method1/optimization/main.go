...
func maxProfit(prices []int) int {
	n := len(prices)
	max_k := 2
	const arrTrade = 3 // 交易次数 len 为 max_k + 1, 因为包括 0

	if n < 1 {
		return 0
	}
	// 状态 天数、交易次数、持有状态  // 交易次数 len 为 max_k + 1, 因为包括 0
	dp := make([][arrTrade][2]int, n) // dp[i][k][0]表示截止到第i天进行了k次交易，目前不持有股票；dp[i][k][1]表示截止到第i天进行了k次交易，目前持有股票.
	// 选择 buy sell rest
