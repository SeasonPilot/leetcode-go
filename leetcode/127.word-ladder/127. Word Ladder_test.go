...
func Test_Problem127(t *testing.T) {
	...
	qs := []question127{
		{
			para127{"ymain", "oecij", []string{"ymann", "yycrj", "oecij", "ymcnj", "yzcrj", "yycij", "xecij", "yecij", "ymanj", "yzcnj", "ymain"}},
			ans127{5},
		},
		{
	...
	for _, q := range qs {
		...
		//fmt.Printf("【input】:%v       【output】:%v\n", p, ladderLengthOptim(p.b, p.e, p.w))
		//fmt.Printf("【input】:%v       【output】:%v\n", p, ladderLength3(p.b, p.e, p.w))
		fmt.Printf("【input】:%v       【output】:%v\n", p, ladderLength4(p.b, p.e, p.w))
	}
	fmt.Printf("\n\n\n")
}
